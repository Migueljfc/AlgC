...
Graph* GraphCopy(const Graph* g) {
  ...
  unsigned short isDigraph = GraphIsDigraph(g);
  ...
  if(isWeighted == 1){
    for(int i = 0;i < numVertices; i++){
      ...
      ver = ListGetCurrentItem(g->verticesList);
      adj = GraphGetAdjacentsTo(g, ver->id);
      for(int k = 0; k <= adj[0]; k++){
          GraphAddEdge(copy, ver->id, *(adj + k));
      }
      free(adj);
    }
  }else{
    for(int i = 0;i < numVertices; i++){
      ...
      ver = ListGetCurrentItem(g->verticesList);
      adj = GraphGetAdjacentsTo(g, ver->id);
      List* edges = ver->edgesList;
      for(int k = 0;k <= adj[0];k++){
        ListMove(g->verticesList, k);
        ed = ListGetCurrentItem(edges);
        GraphAddWeightedEdge(copy, ver->id,*(adj+k),ed->weight);
      }
      free(adj);
    }
  }
...
unsigned short GraphCheckInvariants(const Graph* g) {
  int nEd = 0;
  int nEdVer = 0;
  int verSize = ListGetSize(g->verticesList);
  ListMoveToHead(g->verticesList);
  while(ListGetCurrentPos(g->verticesList) < verSize -1){
    nEdVer = ListGetSize(ListGetCurrentItem(g->verticesList));
    nEd = nEd + nEdVer;
